...
[#6](https://zenn.dev/harieshokunin/articles/openclaw-06-discord-hybrid) では Discord 受信窓口を RPi5 に置き、LLM 推論を Windows タワーへ forward する **(D) ハイブリッド** を採用しました。受信側は動いたのですが、**実運用してみると Pi5 上で OpenClaw daemon を 24h 走らせること自体が無理筋** であることが分かってきました。

本記事はその「気づき」と「撤退判断」のログです。実装そのもの (OpenClaw 停止 + bridge bot 移設) は本記事執筆時点ではまだ実施できていません。撤退の判断と移行の停滞理由を、ジュニアエンジニア向けに **何が起きてどう判断したか** を順に書きます。
...
撤退判断の根拠は症状 1〜3 ですが、本日 (2026-05-17) に実際にもう 1 つの症状を観測したので付記します。**Win Tower 落ち = Discord 全停止** という SPOF 構造です。
...
Win Tower 側は Ollama がきちんと動いているか確認しておきます。Tailscale 越し `curl` で取得した `/api/tags` がこちら。
...
Mac から `curl` して `http=200 / 27ms`、wells から叩いても `17ms` 応答です。**Tailscale 越しでも latency は無視できる** ので、forward の遅さが Pi5 側の問題であって Win Ollama 側ではないことが分かります。
...
- **持続スループットは 6.3〜6.7 tok/s** — Qwen3.6:27B (Q4_K_M) を Win タワー (具体的なスペックは別記事で) で回した値
...
1. **Win Tower の 24h 稼働** — 今は気分で起こしているので、Wake-on-LAN 含めて運用ルールを決める必要があります
2. **新規 Discord Bot Token の発行** — bridge bot 用に既存とは別の Bot を Developer Portal で作る必要があります (既存 token は OpenClaw 用に温存)
3. **wells 側 OpenClaw の停止と mask** — `systemctl stop && disable && mask` で復活を機械的に防ぎます
...
撤退判断は確定したのに本記事執筆時点で実装が止まっている理由を、ジュニアエンジニア向けに整理しておきます。

### ブロッカー 1: Win Tower の 24h 運用方針

今までは「夜は Win Tower を落としていた」のですが、(E) を採用すると **Win 落ち = Discord 沈黙** になります。本日 13:33〜13:47 の 14 分間 ETIMEDOUT を出していたのが正にそれです。
...
- Win Tower 24h 運用方針の確定 (電気代の試算込み)
...
- wells 側で `systemctl stop && disable && mask openclaw-gateway.service`
- 1 週間運用してみての所感と、再度の (E) → (?) 撤退があるかどうか

ジュニアエンジニア向けに 1 行: **「動かしてみないと limit はわからない」が個人開発の本音です**。#5 で hardening まで組んでから #7 で「全部剥がす」判断をするのは無駄に見えるのですが、剥がすかどうかの判断は実際に走らせて症状を見てからでないと下せませんでした。
