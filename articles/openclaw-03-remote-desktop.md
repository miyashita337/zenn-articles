...
- Raspberry Pi OS 13 (trixie) で **RealVNC / xrdp の解説記事は 8 割もう動きません** — Wayland (labwc) 移行で前提が変わったためです
...
そして trixie + Pi5 環境では、**Wayland コンポジタ labwc への移行で従来手順がほぼ全滅** していました。本記事では Pi Connect と WayVNC 直結の 2 ルートを実機で検証して、選び方と落とし穴をまとめていきます。
...
しかし Pi OS 13 (trixie) は **デフォルトのコンポジタが labwc (Wayland)** になり、X11 前提で書かれた RealVNC Server / xrdp / TigerVNC Server が **そのままでは動かない / 非常に手間がかかる** 状態になっています。
...
つまり、**現代の選択肢は実質「Pi Connect」と「WayVNC」の 2 つだけ** です。本記事ではこの 2 つを実機で動かして比較していきます。
